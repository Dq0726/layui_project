...
layui.define('admin', function(exports) {
  ...
  var entryPage = function(fn) {
    ...
    // 单独页面
    if(isIndPage || pathURL === '/uia/login'){ // 此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
      ...
      if (setter.interceptor) {
        ...
        if(!local[setter.request.tokenName]){
          return location.hash = '/uia/login/redirect='+ encodeURIComponent(url.href || ''); // 跳转到登入页
        }
      }
